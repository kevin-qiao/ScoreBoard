package com.qiao.scoreboard.net;
...
import com.qiao.scoreboard.data.StringUtils;
...
public class HTTPRequestHelper {
    ...
    private static final String LOGTAG = "http request helper";
                ...
                public void process(final HttpRequest request, final HttpContext context) throws HttpException,
                    ...
                    for (String key : sendHeaders.keySet()) {
                        if (!request.containsHeader(key)) {
                            Log.d(LOGTAG, " " + HTTPRequestHelper.CLASSTAG + " adding header: " + key + " | "
                                + sendHeaders.get(key));
        ...
        if (requestType == HTTPRequestHelper.POST_TYPE) {
            ...
            if ((params != null) && (params.size() > 0)) {
                ...
                for (String key : params.keySet()) {
                    Log.d(LOGTAG, " " + HTTPRequestHelper.CLASSTAG + " adding param: " + key + " | "
                        + params.get(key));
    ...
    private void execute(HttpClient client, HttpRequestBase method) {
        Log.d(LOGTAG, " " + HTTPRequestHelper.CLASSTAG + " execute invoked");
    ...
    public static ResponseHandler<String> getResponseHandlerInstance(final Handler handler, 
        ...
        final ResponseHandler<String> responseHandler = new ResponseHandler<String>() {
            ...
            public String handleResponse(final HttpResponse response) {
                ...
                if (entity != null) {
                    try {
                        result = StringUtils.inputStreamToString(entity.getContent(), type);
                        bundle.putString("RESPONSE", result);
                        message.what = 0;	//excute success
                        message.setData(bundle);
                        handler.sendMessage(message);
                    } catch (IOException e) {
                        Log.e(LOGTAG, " " + HTTPRequestHelper.CLASSTAG, e);
                        bundle.putString("RESPONSE", "Error - " + e.getMessage());
                        message.what = 1;	//entity get content error
                        message.setData(bundle);
                        handler.sendMessage(message);
                    }
                } else {
                    ...
                    message.what = 2;	//response get entity error
                    message.setData(bundle);
                    handler.sendMessage(message);
                }
                return result;
